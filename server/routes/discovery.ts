...
const entityDatabase: Record<string, DiscoveryContent> = {
  'robert mapplethorpe': {
...
// Hybrid discovery endpoint - searches videos AND provides AI analysis
router.post('/api/discovery/discover', async (req: Request, res: Response) => {
  try {
    ...
    let matchedEntity: DiscoveryContent | null = null;
...
router.get('/api/discovery/entities', (req: Request, res: Response) => {
...
router.get('/api/discovery/entity/:id', (req: Request, res: Response) => {
  const { id } = req.params;
